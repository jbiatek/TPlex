 ...
 *
 */
public class JCuteExternalWorld implements ExternalWorld {
...
//			return lastValue;
			...
			// the old one. 
			Cute.Assume((lastValue != null && newVal.ge(lastValue).isTrue()));
	...
	private String symbolicString(String makeMeSymbolic) {
		...
		throw new RuntimeException("Symbolic strings don't exist in Cute");
	}
	...
	private int nondetIntIndex(int maximum) {
		int index = symbolicInt(0);
		Cute.Assume(index >= 0 && index < maximum);
		return index;
	}
	...
	private PValue lookup(PString stateName) {
		...
		if ( ! currentLookupValues.containsKey(lookup)) {
			if (oldLookupValues.containsKey(lookup) && symbolicBoolean(true)) {
